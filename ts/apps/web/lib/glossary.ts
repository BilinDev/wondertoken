...
export const GLOSSARY = {
  poolNav:
    "Net Asset Value — the protocol's on-chain accounting of the total USDC value backing all WNDR tokens.",
  liquidUsdc:
  ...
  referenceRate:
    "USDC value of one WNDR token = available NAV ÷ token supply. Buys and sells both use this protocol-defined rate.",
  pendingClaims:
    "Total USDC of sell requests that have been filed but not yet settled.",
  totalSupply:
    "All WNDR tokens that currently exist, including tokens held in escrow for pending sell requests. Escrowed tokens are only burned when a sell settles.",
  circulatingSupply:
    "WNDR freely held by users. Filing a sell request moves tokens into escrow and lowers circulating supply; cancelling the request restores them.",
} as const;
