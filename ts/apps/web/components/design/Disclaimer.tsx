...
const STORAGE_KEY = "wndr-disclaimer-ack";
...
export function Disclaimer() {
  ...
  return (
    <div
      className="fixed inset-0 z-[95] flex animate-fade-in items-center justify-center bg-[rgba(4,4,8,0.7)] p-5 backdrop-blur-[6px] max-[839px]:items-end max-[839px]:p-0"
      role="dialog"
      aria-modal="true"
      aria-labelledby="disclaimer-title"
    >
      <div className="relative w-full max-w-[520px] animate-sheet-in overflow-hidden rounded-[18px] border border-line-2 bg-surface-2 shadow-pop max-[839px]:max-w-none max-[839px]:rounded-b-none">
        {/* Violet halo behind the header, mirroring the page canvas */}
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-x-0 -top-24 h-48 bg-[radial-gradient(60%_100%_at_50%_100%,var(--halo),transparent_75%)]"
        />
        ...
        <div className="relative flex flex-col gap-4 p-6 pb-[calc(24px+env(safe-area-inset-bottom))] desk:pb-6">
          ...
          <p className="max-h-[240px] overflow-y-auto rounded-xl border border-line bg-surface px-4 py-3.5 text-[12.5px] leading-relaxed text-ink-2">
            {TEXT}
          </p>
          ...
          <button
            ...
            type="button"
            onClick={accept}
            className="flex h-11 items-center justify-center rounded-[10px] bg-mint-btn text-[14.5px] font-semibold text-mint-ink shadow-glow transition-colors hover:bg-mint-btn-h"
          >
